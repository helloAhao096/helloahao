<template>
  <div class="main">
    ...
    <div class="tags">
      <span
        @click="toggleTag(key,item)"
        v-for="(item, key) in data"
        class="tag"
        :style="getFontSize(data[key].length)"
        :class="{ activeTag: tagStore.tag === key }"
      >
<!--    有多少条该类型的标签    -->
    ...
    <h4 class="header" v-if="tagStore.tag">
      <svg
        t="1641783753540"
        class="fas-icon"
      ...
      <span class="header-text">{{ tagStore.tag }}</span>
    </h4>
    <a
      :href="withBase(article.regularPath)"
      v-for="(article, index) in data[tagStore.tag]"
      :key="index"
      class="article"
    >
      <div class="title">
...
import {useData, withBase, defineConfigWithTheme} from "vitepress";
