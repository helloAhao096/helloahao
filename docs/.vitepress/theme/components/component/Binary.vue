<template>
    <main>
        ...
        <input type="checkbox" id="in-7-4"/>
        <input type="checkbox" id="in-7-5"/>
        <input type="checkbox" id="in-7-6"/>
        <input type="checkbox" id="in-7-7"/>
        ...
        <input type="checkbox" id="in-8-6"/>
        <input type="checkbox" id="in-8-7"/>
        <input type="checkbox" id="in-8-8"/><br/>
        <input type="checkbox" id="in-9-1"/>
        <input type="checkbox" id="in-9-2" checked/>
        <input type="checkbox" id="in-9-3" checked/>
        <input type="checkbox" id="in-9-4"/>
        <input type="checkbox" id="in-9-5"/>
        <input type="checkbox" id="in-9-6"/>
        <input type="checkbox" id="in-9-7"/>
        <input type="checkbox" id="in-9-8" checked/><br/>
        ...
        <input type="checkbox" id="in-10-7" checked/>
        <input type="checkbox" id="in-10-8" checked/><br/>

        <input type="checkbox" id="in-11-1"/>
        <input type="checkbox" id="in-11-2"/>
        <input type="checkbox" id="in-11-3"/>
        <input type="checkbox" id="in-11-4"/>
        <input type="checkbox" id="in-11-5"/>
        <input type="checkbox" id="in-11-6"/>
...
/**
//back
$sel-nav 导航栏，上面的checkbox
$sel-text 是展示的文字，hello world
$sel-label就是选择器主体，钢琴键 修改#in-前面的class即可 ****无法实现****
现在的问题是不能在包着section的情况下选择下面对应的label

在新的scss中不能不使用calc()或者math.div()直接操作除法 /
**/
//sequence
// 有多少组 字节单位
...
@each $item in $map {
  ...
  @for $g from 1 through $seq-count {
    ...
    // 展示的文字
    $sel-text: $sel-text + "~ #binary-output .output-#{$g}:after";
    //导航栏checkbox
    $sel-nav: $sel-nav + " ~ .binary-nav div:nth-child(#{$g}):after";
...
@for $s from 1 through $seq-count {
  ...
  @for $i from 1 through $seq-length {
    // style label
    //label就是选择器主体，钢琴键 修改#in-前面的class即可
...
#{$sel-label} {
  ...
  &::after {
    top: calc($label-h / 2) + calc($label-gutter / 2);
    background: $c-on;
...
nav {
  ...
  div {
    ...
    label {
      ...
      position: relative;
      cursor: pointer;
      box-shadow: //inset 0px 0px 0px 1px lighten($c-dark, 20%),
      ...
      &::after {
        ...
        display: block;
        position: absolute;
        height: calc($label-h / 2) - $label-gutter;
        width: calc(100% - #{$label-gutter});
        left: calc($label-gutter / 2);
        top: calc($label-gutter / 2);
        border-radius: 4px;
        background: $c-off;
